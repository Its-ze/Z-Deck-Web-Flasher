...
def main() -> int:
    ...
    check(all(standard_parts.get(offset) == firmware_path for offset in APP_SLOT_OFFSETS), "standard installer writes the current Z-Deck payload to both A/B slots", failures)
    ...
    check(all(dual_parts.get(offset) == firmware_path for offset in APP_SLOT_OFFSETS), "dual installer writes the current Z-Deck payload to both A/B slots", failures)
