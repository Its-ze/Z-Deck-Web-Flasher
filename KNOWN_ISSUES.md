...
- The firmware base reports `2.8.0.zdeck26`; the public-safe release label is `0.2.25-cyberdeck`.
...
- USB SD mass storage is disabled in `0.2.25-cyberdeck` because the CDC serial + MSC composite path was unstable on Windows during API sessions.
...
- If the right sidebar or top bar still overlaps after an update, confirm the device is actually on `2.8.0.zdeck26`; the fixed gutter is not present in older zdeck24/zdeck25 builds.
- Avoid editing the same SD-card files from the host computer and the T-Deck UI at the same time.
- GPS and map behavior depends on sky view, valid GPS wiring, and available/cached map tiles. Z-Deck 0.2.25 disables the inherited fresh T-Deck power-saving default so supported GPS receivers stay active, keeps map page switching separate from SD tile style loading, persists the default map page, reports missing tiles, and recenters once when late GPS or node coordinates arrive.
