...
- The firmware base reports `2.8.0.zdeck24`; the public-safe release label is `0.2.23-cyberdeck`.
...
- USB SD mass storage is disabled in `0.2.23-cyberdeck` because the CDC serial + MSC composite path was unstable on Windows during API sessions.
...
- GPS and map behavior depends on sky view, valid GPS wiring, and available/cached map tiles. Z-Deck 0.2.23 disables the inherited fresh T-Deck power-saving default so supported GPS receivers stay active, keeps map page switching separate from SD tile style loading, and recenters once when late GPS or node coordinates arrive.
