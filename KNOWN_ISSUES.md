...
- The firmware base reports `2.8.0.zdeck31`; the public-safe release label is `0.2.30-cyberdeck`.
...
- USB SD mass storage is disabled in `0.2.30-cyberdeck` because the CDC serial + MSC composite path was unstable on Windows during API sessions.
...
- If the right sidebar, battery icon, top bar, OTA controls, or map GPS overlay still look wrong after an update, confirm the device is actually on `2.8.0.zdeck31`; the fixed gutter, bounded header lane, Settings OTA block, map GPS recovery, and OTA/backup button repaint fix are not present in older zdeck24-zdeck30 builds.
- Avoid editing the same SD-card files from the host computer and the T-Deck UI at the same time.
- GPS and map behavior depends on sky view, valid GPS wiring, and available/cached map tiles. Z-Deck 0.2.30 keeps the zdeck30 GPS/map recovery behavior and disables the inherited fresh T-Deck power-saving default so supported GPS receivers stay active, keeps map page switching separate from SD tile style loading, persists the default map page, reports missing tiles, shows GPS coordinates when available, and recovers from stale saved map-home data by prioritizing live GPS and positioned mesh nodes.
