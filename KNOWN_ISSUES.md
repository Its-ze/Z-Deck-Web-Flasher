...
- The firmware base reports `2.8.0.zdeck29`; the public-safe release label is `0.2.28-cyberdeck`.
...
- USB SD mass storage is disabled in `0.2.28-cyberdeck` because the CDC serial + MSC composite path was unstable on Windows during API sessions.
...
- If the right sidebar, battery icon, top bar, or OTA controls still look wrong after an update, confirm the device is actually on `2.8.0.zdeck29`; the fixed gutter, bounded header lane, and Settings OTA block are not present in older zdeck24-zdeck28 builds.
- Avoid editing the same SD-card files from the host computer and the T-Deck UI at the same time.
- GPS and map behavior depends on sky view, valid GPS wiring, and available/cached map tiles. Z-Deck 0.2.28 disables the inherited fresh T-Deck power-saving default so supported GPS receivers stay active, keeps map page switching separate from SD tile style loading, persists the default map page, reports missing tiles, and recenters once when late GPS or node coordinates arrive.
