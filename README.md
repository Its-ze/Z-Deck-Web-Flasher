...
Public GitHub Pages flasher for **Z-Deck Firmware Pack 0.2.23-cyberdeck**.
...
- Release label: `Z-Deck 0.2.23-cyberdeck`
- Firmware base version: `2.8.0.zdeck24`
- Target: `t-deck-tft`
...
- Map/position pages: Mesh map, Live compass, DF/Radar, and Distance alert are switched with the Map tab or the map menu without changing the real SD tile style. The map menu also includes Center, which moves to GPS, saved map area, or positioned mesh nodes.
...
- Offline maps: SD preparation creates Z-Deck map folders, while the real tile loader stays on `/maps/zdeck-mesh` by default and filters non-tile page folders from the tile-style dropdown. If GPS or peer locations arrive after opening the map, the view recenters once unless you have already panned, zoomed, or chosen Home.
...
- USB storage: disabled by default in `0.2.23-cyberdeck` so Web Serial and Meshtastic API sessions stay stable; SD prep/journal/ringtone/backup features still use the card internally.
