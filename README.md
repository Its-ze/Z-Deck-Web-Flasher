...
- Map/position pages: Mesh map, Live compass, DF/Radar, and Distance alert are switched with the Map tab or the map menu without changing the real SD tile style. The default page persists across restarts, and the map menu includes Center, which moves to live GPS, positioned mesh nodes, then saved map area. The overlay shows current GPS coordinates when available.
